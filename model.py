...
def variation_to_standard_deviation(
    ...
    if variation_value < 0:
        raise ValueError(
            ...
            "adjustment fields for values below target."
        )
...
def create_filling_targets(
    ...
    """
    Create the complete filling-target range for the three curves.

    The range always includes:
    - target filling;
    - the input filling scenario;
    - the simple pastry-compensation point;
    - at least target filling ±15 g.

    It automatically expands for unusual or extreme scenarios.
    """
...
def run_optimisation(
    ...
    input_index = (
        ...
        .sub(input_filling_target)
        .abs()
        .idxmin()
    )

    input_result = results_df.loc[
        input_index
    ]
    ...
    summary = {
        ...
        "input_good_units": float(
            input_result[
                "good_units"
            ]
        ),
        "optimum_filling_target": float(
            optimum_result[
                "filling_target"
            ]
        ),
        "optimum_filling_adjustment": float(
            optimum_result[
                "filling_adjustment"
            ]
        ),
        "optimum_actual_units": float(
            optimum_result[
                "actual_units"
            ]
        ),
        ...
        "optimum_underweight": float(
            optimum_result[
                "underweight_percentage"
            ]
        ),
        "optimum_seconds": float(
            optimum_result[
                "expected_seconds"
            ]
        ),
        "optimum_good_units": float(
            optimum_result[
                "good_units"
            ]
        ),
